/**
 * debug.view.spritesheet — sprite-sheet stepped-animation visualisation.
 *
 * show(sectorId) lays a half-transparent clone of the WHOLE sprite sheet over
 * each sprite in a sector (the `.sector#s{id}` containers the renderer already
 * groups things under). The clone is appended as a child of the sprite, so it
 * inherits the live sheet metrics (--w/--h/--frames/--cols/--rows/--heading) and
 * the sprite's billboard + mirror transform. Where the real sprite steps
 * background-position to window one cell, the clone shows the whole sheet and
 * translates it by the same amount (driven by --viz-step), so the active cell
 * stays pinned over the opaque original while the sheet drifts.
 *
 ...
 * lives in debug/spritesheet.css; this module never touches the renderer.
 *
 * Exposed as debug.view.spritesheet (a { show, hide } pair) in console.js.
 */
...
// The sheet animations a sprite can run (see spritesheet.css): sprite-cycle is
// the looping walk; sprite-stop / sprite-hide are the one-shot attack/death runs.
...
export const spritesheet = {
    /** Show the sheet ghost over every sprite in a sector (or all sprites if no
     *  id). Skips sprites that already have one. */
    show(sectorId) {
